...
const prompt = '你是利世民的個人情報助理。以下是自今日上一次簡報（午報或早報）以來新增的資料。'
  + '請用繁體中文撰寫一份精簡的晚報更新，聚焦於新增內容的重點與當日整體追蹤。\n\n'
  + '日期：' + dateStr + '（美東時間）\n'
  ...
  + '\n---\n\n請按以下格式撰寫，使用Markdown：\n\n'
  + '# ' + dateStr + ' 晚報（更新）\n\n'
  + '## 晚間重點\n列出自上一次簡報以來最重要的 3 至 7 個新增故事。每條包括：\n- 標題\n- 1-2 句分析\n- 來源及連結\n\n'
  + '## 今日總結\n用 3至4 句話總結今日整體的資訊環境變化、主要主題、以及值得关注的越境信號。\n\n'
  + '---\n\n以下是新增資料：\n\n'
...
WHERE NOT EXISTS (
  SELECT 1 FROM briefings
   WHERE date = (NOW() AT TIME ZONE 'America/New_York')::date AND slot = 'evening'
)`;
...
const saveBriefing = node({
  ...
  version: 2.6,
  config: {
    name: 'Save Evening Briefing',
    parameters: {
      operation: 'executeQuery',
      query: "INSERT INTO briefings (date, slot, markdown, prompt_tokens, output_tokens, generated_at) VALUES ($1::date, 'evening', $2, $3, $4, NOW()) ON CONFLICT (date, slot) DO UPDATE SET markdown = EXCLUDED.markdown, prompt_tokens = EXCLUDED.prompt_tokens, output_tokens = EXCLUDED.output_tokens, generated_at = NOW()",
      options: {
...
export default workflow('generate-briefing-evening', 'Generate Briefing · Evening')
  .add(eveningCron)
  .to(scheduledCutoff)
  .to(fetchDelta)
  .to(buildPrompt)
